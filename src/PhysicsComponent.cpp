...
void PhysicsComponent::initCircle(b2BodyType type, float radius, glm::vec2 center, float density, int16 collisionIndex)
{
    assert(body == nullptr);
    ...
    fxD.shape = circle;
    fxD.density = density;
    fxD.filter.groupIndex = collisionIndex;
    fixture = body->CreateFixture(&fxD);

    SpaceShoot::instance->registerPhysicsComponent(this);
}

void PhysicsComponent::initBox(b2BodyType type, glm::vec2 size, glm::vec2 center, float density, int16 collisionIndex)
{
    assert(body == nullptr);
    autoUpdate = type != b2_staticBody;
    // do init
